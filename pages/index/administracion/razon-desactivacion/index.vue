...
<section class="container-fluid" id="admin-faq">
    <div class="container">
      <div class="row">
        <div class="col-12">
          <h2 class="text-center">Razón de Desactivación de Cuenta</h2>
        </div>
      </div>
      <div class="row">
        ...
        <div class="col-lg-4 offset-md-3 col-md-6 offset-sm-3 py-1">
          <div class="input-group">
            <input class="form-control border border-right-0" name="search" v-model.trim="search" placeholder="Buscar Nombre de Razón de Desactivación..." autocomplete="off" autofocus="autofocus" type="text" @keyup="buscarRazonDes">
...
export default {
  ...
  methods: {
    ...
    buscarRazonDes () {
      ...
      if (this.deacAux.length === 0) {
        this.deacAux = this.paginatedData
      }

      // Si hay algo escrito en el buscador...
      if (this.search.length > 0) {
        // Se buscan todas las razones en que el nombre o parte de ellos posea el texto escrito en el buscador
        let deacSearch = this.deactivationreasons.map(deactivationreason => {
        ...
        let deacFound = []
        deacSearch.forEach(deactivationreason => {
          if (deactivationreason) deacFound.push(deactivationreason)
        })

        // Ordena el listado obtenido en orden lexicografico.
        deacFound.sort(function (a, b) {
        ...
        custompaginator.paginate(deacFound)
          .then(({ paginatedData }) => {
      ...
      if (this.search.length === 0) {
        this.paginatedData = this.deacAux
        this.pagination = 0
